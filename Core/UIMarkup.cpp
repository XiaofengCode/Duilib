...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        ...
        *pstrText++ = _T('\0');
        ...
        // Parse attributes
        if( !_ParseAttributes(pstrText) ) return _Failed(_T("Error parsing element attributes"), pstrText);
        _SkipWhitespace(pstrText);
        ...
        else
        {
            ...
            if( !_ParseData(pstrText, pstrDest, _T('<')) ) return _Failed(_T("Error parsing element data"), pstrText);
...
{   
	...
	if( *pstrText == _T('/') ) return true;//修复当控件未设置属性时加载资源失败的BUG
    *pstrText++ = _T('\0');
    _SkipWhitespace(pstrText);
